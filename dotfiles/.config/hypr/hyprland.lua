...
-- Layout: monitores externos lado a lado no topo, laptop centralizado embaixo.
-- A posição do laptop se recalcula automaticamente conforme quantos externos
-- estão conectados (0, 1 ou 2) — ao carregar a config e a cada plug/unplug de
-- monitor (via hl.on("monitor.added"/"monitor.removed", ...)).
-- Ver docs/monitores-setup.md.
...
local function applyMonitorLayout()
    ...
    -- Quando um monitor some, o(s) monitor(es) sobrevivente(s) que mudam de
    -- posição não "empurram" automaticamente as layer-shell surfaces já
    -- abertas nele (waybar, wallpaper) — elas ficam com a geometria antiga
    -- até algo forçar um reflow. force_renderer_reload() faz isso na hora.
...
-- WhatsApp — duas contas isoladas via perfis Chromium (ver docs/whatsapp-chromium-setup.md)
...
-- WhatsApp — sempre abrem no workspace 9, sem roubar o foco (ver docs/whatsapp-chromium-setup.md)
hl.window_rule({
